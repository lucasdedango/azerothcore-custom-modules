...
namespace ServerCustomization
{
    ...
    {
        handler->PSendSysMessage("|cff00ff00Automation:|r mode |cffffff00%s|r enabled.", mode);
    }
    ...
    static bool StartGrind(ChatHandler* handler, uint32 targetEntry = 0, char const* targetName = nullptr)
    {
        ...
        if (targetEntry)
        {
            handler->PSendSysMessage("|cff00ff00Automation:|r grind target enabled - %s (entry %u).",
    ...
    public:
        ...
        static bool HandleDamageDebug(ChatHandler* handler, std::string_view args)
        {
            ...
            {
                handler->PSendSysMessage("|cffffff00Damage debug:|r %s.", state->enabled ? "|cff00ff00ON|r" : "|cffff4444OFF|r");
                return true;
            }
        ...
        static bool HandleAutoStatus(ChatHandler* handler)
        {
            ...
            switch (GetAutomationMode(player))
            {
                case ServerCustomizationGatherOnly::Mode::Mining:
                    handler->PSendSysMessage("|cff00ff00Automation:|r active - mine%s. Target spawn: %u", (state && state->suppressMobLoot) ? " noloot" : "", state ? uint32(state->targetSpawnId) : 0);
                    break;
                case ServerCustomizationGatherOnly::Mode::Herbalism:
                    handler->PSendSysMessage("|cff00ff00Automation:|r active - herb%s. Target spawn: %u", (state && state->suppressMobLoot) ? " noloot" : "", state ? uint32(state->targetSpawnId) : 0);
                    break;
                case ServerCustomizationGatherOnly::Mode::Gathering:
                    handler->PSendSysMessage("|cff00ff00Automation:|r active - gather%s. Target spawn: %u", (state && state->suppressMobLoot) ? " noloot" : "", state ? uint32(state->targetSpawnId) : 0);
                    break;
                case ServerCustomizationGatherOnly::Mode::Grind:
                    if (state && state->grindCreatureEntry)
                        handler->PSendSysMessage("|cff00ff00Automation:|r active - grind target entry %u.", state->grindCreatureEntry);
                    else
